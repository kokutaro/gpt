...
FILE_BASE_PATH = os.environ.get("FILE_BASE_PATH", "/var/data/doc-data")
EMB_KEY = "{%s}embed" % NS["r"]
ANCHOR_LIST = ["xdr:oneCellAnchor", "xdr:twoCellAnchor", "xdr:absoluteAnchor"]
IMG_BASE_PATH = os.path.join(FILE_BASE_PATH, "img")
    ...
    for i, d in enumerate(drawings):
        for autoshape in _enumerate_autoshapes(d):
            ...
            for emb in autoshape.findall(".//a:blip", NS):
                ...
                shape_infos.append(
                    {"type": "image", "img_id": img_id, "coord": shape_coord}
                )
...
def load_xlsx(file_path: str, uuid: str) -> List[Document]:
    ...
    # 作業ディレクトリの設定
    work_dir = "tmp/work"
    tmp_path = os.path.join(FILE_BASE_PATH, work_dir)
    if not os.path.exists(tmp_path):
        ...
        for i, cell_value in enumerate(cell_values):
            for r in filter(lambda x: x == i, closest_nodes):
                ...
                if shape_info["type"] == "image":
                    metadata["img_ids"].append(
                        {"line_no": index, "img_id": shape_info["img_id"]}
                    )
            lines_with_img_path.append(cell_value["value"])

        # ドキュメントを作成
        page_content = "\n".join([cell["value"] for cell in cell_values])
        documents.append(
            Document(
